...
class V3PrecisionKnobTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      # The eight always-on flags (fast_mask, gate_stitch, merge_kv_dma,
      # fuse_qk, flat_q, static_kv_dst, disable_bounds_checks and the two-step
      # PV deferral) are no longer configurable -- they are unconditional
      # kernel behaviour. Their cases collapsed onto the default-config runs
      # below, which now exercise all of them on every shape here rather than
      # one flag at a time. What is still worth varying is the shape, the
      # mask, and the flags that remain.
      ("baseline", {}, None, None),
      ("p_same_dtype_as_v", dict(p_same_dtype_as_v=True), None, None),
      # The causal predicate has a single-token fast path and a general form;
      # the sliding-window inequality is rearranged between them
      # (`q_kv_diff < sw + offset` becomes `kv_iota > -offset - sw`), so both
      # need a windowed case.
      ("general_mask_sliding_window", {}, 512, None),
      # Prefill shape. With `num_queries_per_block=1` the prefill band also
      # has bq_sz==1, so the single-token path applies here too.
      ("general_mask_prefill", {}, None, [(256, 1024)]),
      # And with bq_sz > 1, where that path must not engage at all. `bq16` in
      # the name is what selects `num_queries_per_block=16` below.
      ("prefill_bq16", {}, None, [(256, 1024)]),
  )
...
class V3BlockOverrunTest(parameterized.TestCase):
  ...
  Found while validating the single-token mask path: a prefill shape with
...
class V3CompactKvDimTest(parameterized.TestCase):
  """A compact KV cache must be a pure layout change, not a numerics change.
  ...
  @parameterized.named_parameters(
      ("decode", [(1, 1024), (1, 768), (1, 2048)]),
      ("chunked_prefill", [(256, 1024)]),
  )
  def test_compact_matches_reference(self, seq_lens):
    _skip_unless_tpu(self)
    from tokamax._src.ops.experimental.mla import v3_op  # pylint: disable=g-import-not-at-top

...
class MlaV3DecodePerfTest(absltest.TestCase):
  ...
  Each flag was measured individually (kernel time, device):

      fast_mask               -3.4%   single-token causal-mask fast path
      disable_bounds_checks   -6.3%   v2 already compiles with these off
      merge_kv_dma            -3.0%   one DMA per cache page instead of two
  ...
  (+49%). `two_step_flash_attention` is inert here (`q_split == 1`).

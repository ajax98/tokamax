...
def store_new_kv_lane(
  ...
  if cfgs.block.bq_sz == 1:
    ...
    # Overwrite all rows of the target page with clean tokens so trailing padding cannot poison dot products.
    for r_offset in range(k_rows):
      r = dst_page_start + r_offset
      existing_vreg = strided_vmem_ref[pl.ds(r, outer_dim, lanes_per_col)]
      new_row = jax.lax.select(
          r < dst_chunk_idx, existing_vreg, merged_dst_vreg
      )

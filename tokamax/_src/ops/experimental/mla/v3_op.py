...
class Config:
  ...
  # Ported from v2. Defaults reproduce original v3 behaviour, so an untuned
  # v3 is unchanged and these only take effect when explicitly enabled.
  # `narrow_scores` is a bool rather than a dtype so the Config stays trivially
  # hashable and pydantic-serialisable; True means bf16, matching v2's default.
  narrow_scores: bool = False
  p_same_dtype_as_v: bool = False
  two_step_flash_attention: bool = False
  # Single-token mask fast path; see `ServingConfigs.fast_mask`.
  fast_mask: bool = False
  # Sublane-granular KV padding (640 -> 576 for DeepSeek-V3); see
  # `ServingConfigs.compact_kv_dim`. Requires a matching cache shape, so it is
  # only usable with a `cache_layout='v3_compact'` arg-spec.
  compact_kv_dim: bool = False
  # DMA / VMEM knobs; see the corresponding `ServingConfigs` fields.
  tight_kv_slack: bool = False
  merge_kv_dma: bool = False
  disable_bounds_checks: bool = False
  # Stride-shaping pad; see `ServingConfigs.kv_slack_pad_lanes`.
...
class V3MultiHeadLatentAttention(base.MultiHeadLatentAttention):
  ...
  def _fwd(
    ...
    # The `Op` contract is token-major `[T, H, L]`; v3, like v2, takes and
    # returns head-major `[H, T, L]`. `q_pe` is token-major in both.
    out, updated_cache_kv_v3 = mla_wrapper.mla_ragged_paged_attention(
        ...
        s_dtype=jnp.bfloat16 if config.narrow_scores else None,
        p_same_dtype_as_v=config.p_same_dtype_as_v,
        two_step_flash_attention=config.two_step_flash_attention,
        fast_mask=config.fast_mask,
        compact_kv_dim=config.compact_kv_dim,
        tight_kv_slack=config.tight_kv_slack,
        merge_kv_dma=config.merge_kv_dma,
        disable_bounds_checks=config.disable_bounds_checks,

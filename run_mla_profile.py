...
def profile_one(spec, impl_name, out_dir, *, iterations, warmup, configs_dir,
  ...
  if overrides:
    ...
    for item in overrides.split(","):
      ...
      # Type from the current value rather than the annotation: pydantic
      # dataclass `.type` may be a string, and Config now mixes int, bool and
      # str fields (`bands`).
      current = kwargs[k]
    ...
    # The trace directory is one path component, so it is capped at 255 bytes.
    # With enough knobs set the spelled-out variant blows past that: two runs
    # then land in the *same* directory and the second silently overwrites the
    # first, which looks like "only one trace was written" rather than an
    # error. Fall back to a digest, and print the legend in the summary so the
    # mapping stays recoverable.
    if len(spec.name) + len(impl_name) + len(label_suffix) > 180:
      label_suffix = "__h" + hashlib.sha1(overrides.encode()).hexdigest()[:10]
  ...
  # `ql_nope` is donated for the same reason, and it matters just as much on
  # the large prefill shapes. v3's `pallas_call` aliases `ql_nope` to the
  # output, so if the caller still owns that buffer XLA emits a defensive copy
  # of it. On `prefill_bf16`, `ql_nope` is [4096, 128, 512] bf16 = 512 MB and
  # the copy shows up as a pure *identity* -- same shape, same
  # `{2,1,0:T(8,128)(2,1)}` layout, source to destination -- costing 0.333 ms,
  # 9% of v3's total, for no work at all.
  #
  # It is also asymmetric, which is what makes it dangerous rather than merely
  # wasteful: v2 does not alias q to its output, so it never pays this, while
  # it does pay two genuine 512 MB `xpose_pipeline` transposes (0.71 ms) for
  # its head-major contract. Leaving `ql_nope` undonated charges v3 for a
  # harness artifact and v2 for nothing.
  #
  # Donating means the output aliases the query buffer, so the next iteration
  # must consume the output as its query -- the same chaining already used for
  # `cache_kv`. The values drift from iteration to iteration; the shapes,
  # layouts and work do not, which is what is being timed.
  donated = ("cache_kv", "ql_nope")
  other_args = {k: v for k, v in args.items() if k not in donated}

  @functools.partial(jax.jit, donate_argnames=donated)
  def run(cache_kv, ql_nope, other):
    return tuned_op(cache_kv=cache_kv, ql_nope=ql_nope, **other)

  # Warm up outside the trace so compilation does not land in it.
  cache = args["cache_kv"]
  q = args["ql_nope"]
  for _ in range(warmup):
    (out, cache) = run(cache, q, other_args)
    q = out
...
def main() -> int:
  ...
  for s in summaries:
    ...
    print(
        ...
        f"  dir={s.get('label_suffix', '')}"
    )
    if s.get("overrides"):
      print(f"{'':33s}  variant: {s['overrides']}")
